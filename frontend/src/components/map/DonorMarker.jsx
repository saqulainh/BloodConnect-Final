        ...
        if (typeof window !== 'undefined' && L) {
            ...
            const htmlString = `
                <div class="relative flex items-center justify-center w-8 h-8 rounded-full shadow-lg border-2 border-white/20 transform transition-all hover:scale-125" 
                    ...
                    ${donor.isOnline ? '<div class="absolute -top-1 -right-1 w-3 h-3 bg-blue-500 border-2 border-white rounded-full animate-ping"></div>' : ''}
                </div>
            `;
    ...
    return (
        <Marker position={[lat, lng]} icon={icon}>
            <Popup className="eims-popup rounded-2xl overflow-hidden border-0">
                <div className="p-4 bg-slate-900 text-white min-w-[160px] flex flex-col gap-2">
                    ...
                    <p className="text-[11px] font-medium text-slate-400">Donor: {donor.name}</p>
                    <button className="w-full bg-slate-800 hover:bg-slate-700 text-white py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-colors mt-1">Message</button>
                </div>
            </Popup>
        </Marker>
    );
